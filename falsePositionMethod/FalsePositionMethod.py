...
class FalsePositionMethod:
    """
    False Position Method solves any with single variable equation with defined set of
    """
    ...
    def solve_single_root(self):
        """
        False Position method for solving a root of a function f
        """
        ...
        while self.current_iteration < self.maximum_iterations and self.should_continue(self):
            self.on_next(self)
            self.x_old = self.x
            self.x = (self.x_low * self.f(self.x_high) - self.x_high * self.f(self.x_low)) / \
                     (self.f(self.x_high) - self.f(self.x_low))
            if self.f(self.x) == 0:
                return self.x
...
def absolute_relative_error_approx(b):
    """
    Continuity interceptor for false position method based on absolute relative error
    approximation
    :param b: FalsePositionMethod object reference
    :return: whether false position method should continue to find root
    """
...
"""
Continuity Interceptors for false position method
